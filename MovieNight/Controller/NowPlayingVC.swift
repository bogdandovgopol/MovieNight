...
class NowPlayingVC: UIViewController {
    ...
    func loadMovies() {
        ...
        MovieService.getMovies(url: TMDB.NOW_PLAYING_BASEURL) { (result, error) in
...
extension NowPlayingVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: SEGUAES.NOW_PLAYING_TO_DETAIL, sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == SEGUAES.NOW_PLAYING_TO_DETAIL {
